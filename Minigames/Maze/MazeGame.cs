...
namespace CompSci_NEA.Minigames.Maze
{
    public class MazeGame : Scenes.Scene
    {
        ...
        private MPathfinder _pathfinder;

        public MazeGame(Main game)
        {
            this.game = game;
            _pathfinder = new MPathfinder();
        }

        public override void LoadContent()
        {
            _grid = new MGrid(game.GraphicsDevice, 111, 83);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                _optimalPath = _pathfinder.BFSPath(
                    _grid.MazeData,
                    _grid.Rows,
                    _grid.Cols,
                    new Point(1, _grid.Rows - 1),
                    exitCell,
                    new HashSet<(Point, Point)>()
                );
                if (_optimalPath != null)
                {
